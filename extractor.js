...
async function getContactLinks(baseUrl) {
  try {
    ...
    const internal = links
      .filter((href) => ["contact", "about", "kontakt"].some((x) => href.toLowerCase().includes(x)))
...
async function processRow(row, headers) {
  console.log(row)
  const skip = (row[headers["skip?"]] || "").toString().trim().toUpperCase() === "TRUE";
...
if (isMainThread) {
  (async () => {
    ...
    console.log("✅ Updates saved to organizations.xlsx");
  })();
}
